...
getTopCoins(10);
// have to cancel thise because coingecko added rate limit
...
thirtyDays.addEventListener("click", () => {
  getCoinHistory(coinLeft, 30, "daily", "single", drawGoogleChart).then(() => {
    days = 30;
    interval = "daily";
  });
});
...
oneYear.addEventListener("click", () => {
  getCoinHistory(coinLeft, 365, "daily", "single", drawGoogleChart).then(() => {
    days = 365;
    interval = "daily";
  });
});
...
function getTopCoins(num) {
  fetch(
    `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=${num}&page=1&sparkline=false`
  )
    .then((res) => res.json())
...
function showDataMain(data, s) {
  ...
  coin24High.textContent = `$${data.market_data.high_24h.usd.toLocaleString(
    undefined,
    { minimumFractionDigits: 2 }
  )}`;
  coin24Low.textContent = `$${data.market_data.low_24h.usd.toLocaleString(
    undefined,
    { minimumFractionDigits: 2 }
  )}`;
  coinAllTimeHigh.textContent = `$${data.market_data.ath.usd.toLocaleString(
    undefined,
    { minimumFractionDigits: 2 }
  )}`;
  coinAllTimeLow.textContent = `$${data.market_data.atl.usd.toLocaleString(
    undefined,
    { minimumFractionDigits: 2 }
  )}`;
  coin24Change.textContent =
    data.market_data.price_change_percentage_24h > 0
      ? `+${data.market_data.price_change_percentage_24h}%`
      : `${data.market_data.price_change_percentage_24h}%`;
  coin24Change.style.color =
    data.market_data.price_change_percentage_24h >= 0 ? "green" : "red";
}
...
function drawGoogleChart(dataArr, coinId, days, interval, position) {
  ...
  const output =
    interval === "daily"
      ? [[`${days}days Data`, coinName]]
      : [["24 hrs Data", coinName]];
  ...
  function drawChart() {
    ...
    const title =
      interval === "daily" ? `${days} Days Data` : "Last 24 hrs Data";
...
function drawGoogleChartMerge(pricesLeft, pricesRight, days, interval) {
  ...
  const coinNameLeft =
    coinLeft.substring(0, 1).toUpperCase() + coinLeft.substring(1);
  const coinNameRight =
    coinRight.substring(0, 1).toUpperCase() + coinRight.substring(1);
  ...
  output =
    interval === "daily"
      ? [[`${days}days Data`, coinNameLeft, coinNameRight]]
      : [["24 hrs Data", coinNameLeft, coinNameRight]];
  for (let i = 0; i < updatedPricesLeft.length; i++) {
    if (interval === "daily") {
      output.push([
        `${updatedPricesLeft[i].month}/${updatedPricesLeft[i].day}`,
        updatedPricesLeft[i].price,
        updatedPricesRight[i].price,
      ]);
    } else {
      output.push([
        updatedPricesLeft[i].hour,
        updatedPricesLeft[i].price,
        updatedPricesRight[i].price,
      ]);
    }
  ...
  function drawChart() {
    ...
    const chart = new google.visualization.AreaChart(
      document.getElementById(`chart-merge`)
    );
    chart.draw(data, options);
  }
}
